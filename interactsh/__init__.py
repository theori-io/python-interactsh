    ...
    async def _poll_worker(self) -> None:
        ...
        while not self._closed and not self._stop_event.is_set():
            ...
            try:
                ...
                logger.debug(
                    f"Poll #{poll_count}: Received {len(interactions)} interactions"
                )
                for i, interaction in enumerate(interactions):
                    logger.debug(
                        f"Poll #{poll_count}: Queueing interaction {i + 1}: {interaction.protocol} from {interaction.remote_address}"
                    )
            ...
            while True:
                try:
                    return await asyncio.wait_for(
                        self._interaction_queue.get(), timeout=0.1
                    )
                except asyncio.TimeoutError:
                    if self._closed and self._interaction_queue.empty():
                        raise StopAsyncIteration
...
class InteractshClient:
    ...
    async def _initialize_new_session(self) -> None:
        ...
        payload = self._encode_registration_request(
            public_key_str, self.secret_key, self.correlation_id
        )
    ...
    def _encode_registration_request(
        self, public_key: str, secret_key: str, correlation_id: str
    ) -> bytes:
    ...
    async def _parse_server_urls(self, server_urls: str, payload: bytes) -> None:
        ...
        raise RegistrationError(
            f"Could not register to any server: {'; '.join(errors)}"
        )
        ...
        async with self.http_client.post(
            ...
            if response.status == 401:
                raise AuthenticationError(
                    "Invalid token provided for interactsh server"
                )
            ...
            if data.get("message") != "registration successful":
                raise RegistrationError(
                    f"Could not get register response: {data.get('message', 'unknown error')}"
                )
    ...
    async def poll_once(self) -> list[Interaction]:
