...
class TestInteractshClient:
    ...
    async def test_client_initialization_default(self):
        ...
        with patch("aiohttp.ClientSession.post") as mock_post:
            ...
            mock_response.json = AsyncMock(
                return_value={"message": "registration successful"}
            )
    ...
    async def test_interact_with_interactions(self):
        ...
        with (
            ...
            mock_reg_response.json = AsyncMock(
                return_value={"message": "registration successful"}
            )
            ...
            mock_poll_response.json = AsyncMock(
                return_value={
                    "data": [],  # Empty for simplicity
                    "extra": [
                        '{"protocol": "http", "unique-id": "test123", "full-id": "test123.example.com", "remote-address": "1.2.3.4", "timestamp": "2023-01-01T00:00:00Z"}'
                    ],
                    "aes_key": "dummy_key",
                    "tlddata": [],
                }
    ...
    async def test_interact_with_timeout(self):
        ...
        with (
            ...
            mock_poll_response.status = 200
            mock_poll_response.json = AsyncMock(
                return_value={"data": [], "extra": [], "aes_key": "", "tlddata": []}
            )
            mock_get.return_value.__aenter__.return_value = mock_poll_response
    ...
    async def test_keep_alive_functionality(self):
        ...
        with patch("aiohttp.ClientSession.post") as mock_post:
            ...
            options = Options(server_url="https://oast.fun", keep_alive_interval=0.1)
    ...
    async def test_session_closed_client(self):
        ...
        with patch("aiohttp.ClientSession.post") as mock_post:
            ...
            with pytest.raises(ClientStateError, match="closed"):
                async with client.interact() as _session:
                    pass
...
class TestIntegration:
            ...
            async with client.interact(poll_interval=1.0) as session:
                async with asyncio.timeout(3.0):  # Poll for 3 seconds
                    async for interaction in session:
                        interactions_received.append(interaction)
                        print(
                            f"Received {interaction.protocol} interaction from {interaction.remote_address}"
                        )
        except asyncio.TimeoutError:
    ...
    async def test_real_interaction_with_http_request(self):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    http_url, timeout=aiohttp.ClientTimeout(total=10)
                ) as response:
                    print(
                        f"HTTP request completed with status: {response.status}, {await response.text()}"
                    )
        ...
        try:
            async with client.interact(poll_interval=1.0) as session:
                async with asyncio.timeout(5.0):  # Poll for up to 10 seconds
                    async for interaction in session:
                        print(
                            f"Received interaction: {interaction.protocol} from {interaction.remote_address}"
                        )
                        interactions_received.append(interaction)
        ...
        print(
            f"Successfully received HTTP interaction from {interaction.remote_address}"
        )
